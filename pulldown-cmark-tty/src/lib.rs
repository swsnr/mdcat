...
//!
//! Consume [`pulldown_cmark`] events and render them onto TTYs.
//!
//! ## Semver guarantees
//!
//! Semver guarantee cover only the public API of this crate; they do explicitly not
//! cover the output format itself.  The output format follows the needs and requirements
//! of the `mdcat` tool and the author's personal preferences, and may change _any time_, even
//! in patch releases.
//!
//! ## Supported features
//!
//! This crate requires a modern VT100 terminal emulator with support for italics, strike-through
//! and OSC-8 l/org/freedesktop/portal/desktopa.
//!
//! On certain terminal emulators it will enable additional features.  In particular,
//! it prints images in Wezterm, iTerm2, Terminology, and Kitty.
//!
//! It will automatically fetch images from HTTP URLs if permitted, and renders SVGs with the resvg
//! library.
//!
//! ## Feature flags
//!
...
//!
//! - `native-tls`: Use the system's native SSL library for HTTPS; on Linux this links against
//!   OpenSSL (enabled by default).
//! - `rustls`: Use the `rustls` library instead of the native SSL library.  This allows for a
//!   fully static build, but still uses the system's CA bundles.
